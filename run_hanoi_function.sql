...
CREATE OR REPLACE FUNCTION get_simple_monthly_salary_hanoi(
    ...
    p_so_thang_da_chia INTEGER DEFAULT 5
)
RETURNS TABLE(
    ...
    cumulative_total_salary NUMERIC,
    ...
    quy_luong_con_lai_duoc_chia NUMERIC
)
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    final_data AS (
        SELECT 
            ...
            COALESCE(acsd.cumulative_ft_salary, 0) AS cumulative_ft_salary,
    ...
    SELECT 
        ...
        fd.cumulative_total_salary,
        fd.cumulative_total_salary AS total_salary_2025,
        -- Quỹ cứng 2025 từ bảng Chi_tieu_2025 (giống bảng so sánh chỉ tiêu 2025)
        (SELECT COALESCE(CAST(REPLACE(ct."KPI_quy_luong_2025"::text, ',', '') AS NUMERIC), 0)
        ...
        -- Quỹ lương còn lại được chia = (Quỹ cứng 2025 - Tổng lương 2025) / Số tháng còn lại
        -- Sử dụng logic giống bảng so sánh chỉ tiêu 2025
        -- Đặc biệt cho "Hệ thống KCB ngoại viện": sử dụng cumulative_ft_salary (chỉ FT salary tích lũy)
        CASE 
            WHEN (12 - p_so_thang_da_chia) > 0 AND 
                 (SELECT COALESCE(CAST(REPLACE(ct."KPI_quy_luong_2025"::text, ',', '') AS NUMERIC), 0)
                  FROM "Chi_tieu_2025" ct
                  WHERE TRIM(LOWER(
                      (string_to_array(ct."Địa điểm_ngành dọc", '>'))[array_length(string_to_array(ct."Địa điểm_ngành dọc", '>'), 1)]
                 ...
                 (CASE 
                    WHEN LOWER(fd.department_key) LIKE '%hệ thống kcb ngoại viện%' 
                    THEN fd.cumulative_ft_salary IS NOT NULL
                    ELSE fd.cumulative_total_salary IS NOT NULL
                  END)
            THEN ((SELECT COALESCE(CAST(REPLACE(ct."KPI_quy_luong_2025"::text, ',', '') AS NUMERIC), 0)
                   FROM "Chi_tieu_2025" ct
                   WHERE TRIM(LOWER(
                       (string_to_array(ct."Địa điểm_ngành dọc", '>'))[array_length(string_to_array(ct."Địa điểm_ngành dọc", '>'), 1)]
                   )) = LOWER(fd.department_key)
                   ...
                   (CASE 
                      WHEN LOWER(fd.department_key) LIKE '%hệ thống kcb ngoại viện%' 
                      THEN fd.cumulative_ft_salary
                      ELSE fd.cumulative_total_salary
                    END)) / (12 - p_so_thang_da_chia)
            ELSE 0
        END AS quy_luong_con_lai_duoc_chia
    FROM final_data fd
...
-- Test function - Kiểm tra quỹ lương còn lại được chia
SELECT 
    department_name,
    quy_cung_2025,
    total_salary_2025,
    quy_luong_con_lai_duoc_chia,
    (quy_cung_2025 - total_salary_2025) / (12 - 5) AS manual_calculation
FROM get_simple_monthly_salary_hanoi(2025, 7, 5) 
