...
import { Loader2, AlertTriangle, DollarSign, TrendingUp, TrendingDown, Minus } from 'lucide-react';
  ...
  const fetchData = useCallback(async () => {
    ...
    for (let i = 0; i < years.length; i++) {
      ...
      try {
        ...
        console.error(`Error fetching part-time salary for ${year}:`, err);
        results[i] = null;
      }
    }
    ...
    return (
      <Card className="h-full">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 pt-3">
          <CardTitle className="text-sm font-semibold text-muted-foreground">Lương PT (24 vs 25)</CardTitle>
  ...
  if (error) {
    return (
      <Card className="border-destructive/50 h-full">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 pt-3">
          <CardTitle className="text-destructive text-sm font-semibold">Lỗi Lương PT (SS)</CardTitle>
