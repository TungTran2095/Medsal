...
import { Loader2, AlertTriangle, LineChart as LineChartIcon, DollarSign } from 'lucide-react';
...
const chartConfig = {
  backOfficeSalaryRatio: {
    label: 'Tỷ lệ lương fulltime khối back',
    color: 'hsl(var(--chart-3))',
    icon: DollarSign,
  },
} satisfies ChartConfig;

interface MonthlySalaryRatioData {
  year: number;
  month_label: string;
...
interface BackOfficeSalaryRatioTrendChartProps {
...
export default function BackOfficeSalaryRatioTrendChart({ selectedYear }: BackOfficeSalaryRatioTrendChartProps) {
  const [chartData, setChartData] = useState<ChartDataPoint[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<ChartError | null>(null);
  const [filterDescription, setFilterDescription] = useState<string>("tỷ lệ lương fulltime khối back theo tháng");
  ...
  const fetchData = useCallback(async () => {
    ...
    setFilterDescription(`tỷ lệ lương fulltime khối back theo tháng (${yearToUse} vs ${yearToUse - 1})`);

    try {
      const functionName = 'get_back_office_salary_ratio_by_month';
      ...
      const typedData = data as MonthlySalaryRatioData[];
      ...
      typedData.forEach(item => {
        ...
        if (item.year === yearToUse) {
          monthData.current = item.back_office_salary_ratio;
        } else if (item.year === yearToUse - 1) {
          monthData.previous = item.back_office_salary_ratio;
        }
      });
      ...
      if (err.type === 'rpcMissing') {
        ...
        setError({ type: 'generic', message: err.message || 'Không thể tải dữ liệu tỷ lệ lương fulltime khối back theo tháng.' });
      }
      console.error("Error fetching back office salary ratio trend:", err);
      setChartData([]);
    } finally {
      setIsLoading(false);
    }
      ...
      <Card className="h-full">
        <CardHeader className="pb-2 pt-3">
          <CardTitle className="text-base font-semibold flex items-center gap-1.5">
            ...
            Xu hướng tỷ lệ lương fulltime khối back
          </CardTitle>
  ...
  return (
    <Card className="h-full">
      <CardHeader className="pb-2 pt-3">
        ...
        <CardDescription className="text-xs truncate" title={filterDescription}>
          Tỷ lệ lương fulltime khối back cho {filterDescription}.
        </CardDescription>
      </CardHeader>
      <CardContent className="pt-2">
          ...
          <ResponsiveContainer width="100%" height="100%">
            <DynamicComposedChart data={chartData} margin={{ top: 15, right: 10, left: 0, bottom: 5 }}>
              ...
              <Line
                ...
                type="monotone"
                dataKey="currentYear"
                stroke={chartConfig.backOfficeSalaryRatio.color}
                strokeWidth={2}
                dot={{ r: 3 }}
                activeDot={{ r: 5 }}
                name={`Tỷ lệ lương fulltime khối back (${currentYear})`}
              ...
              <Line
                ...
                dot={{ r: 2 }}
                activeDot={false}
                name={`Tỷ lệ lương fulltime khối back (${currentYear - 1})`}
                isAnimationActive={false}
                label={undefined}
              />
