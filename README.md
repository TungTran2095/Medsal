...
5.  Click **Run** for each function.
    *   **For `get_monthly_salary_trend_fulltime`, `get_monthly_salary_trend_parttime`, `get_monthly_revenue_trend`, or `get_salary_revenue_ratio_components_by_location`**: If you encounter an error like "cannot change return type of existing function", you MUST first run `DROP FUNCTION function_name(parameters);` (e.g., `DROP FUNCTION get_monthly_salary_trend_fulltime(integer);` or `DROP FUNCTION get_salary_revenue_ratio_components_by_location(integer, integer[]);`) and then run the `CREATE OR REPLACE FUNCTION` script for it.
...
BEGIN
    ...
    --    - "thangpro" (TEXT): Numeric month as text (e.g., '01', '1', '12'). Used for sorting months chronologically.
    --    - "Thang_x" (TEXT): Display label for the X-axis (e.g., 'Tháng 01'). This MUST match the format of the 'thang' column in the "Fulltime" table for joining.
    ...
    INNER JOIN
        "Time" t ON f.nam::INTEGER = t."Năm"::INTEGER -- Join on year
                 AND f.thang = t."Thang_x"            -- Join on month text label (e.g., "Tháng 01" from Fulltime matches "Tháng 01" from Time."Thang_x")
    WHERE
...
BEGIN
    ...
    -- 1. "Time" (capital T) table exists with "Năm" (INTEGER/INT8), "thangpro" (TEXT for sorting), "Thang_x" (TEXT for X-axis and joining).
    ...
    INNER JOIN
        "Time" t ON pt."Nam"::INTEGER = t."Năm"::INTEGER
                 AND pt."Thoi gian" = t."Thang_x" -- Join on month text label
    WHERE
...
BEGIN
    ...
    --    - "Năm" (INTEGER or INT8): Numeric year.
    --    - "thangpro" (TEXT): Numeric month as text (e.g., '01', '12'). Used for sorting months chronologically.
    --    - "Thang_x" (TEXT): Display label for X-axis (e.g., 'Tháng 01'). Must match '"Tháng"' in "Doanh_thu" for joining.
    ...
    INNER JOIN
        "Time" t ON dr."Năm"::INTEGER = t."Năm"::INTEGER
                 AND dr."Tháng" = t."Thang_x" -- Join on month text label
    WHERE
...
This function calculates the full-time and part-time salary components of the salary-to-revenue ratio for each work location, applying optional year and month filters.
...
BEGIN
    -- IMPORTANT ASSUMPTIONS:
    -- 1. "Fulltime" table has "dia_diem" (TEXT), "tong_thu_nhap" (numeric convertible), "nam" (INTEGER), "thang" (TEXT, e.g., 'Tháng 01').
    -- 2. "Parttime" table has "Don vi" (TEXT), "Tong tien" (numeric convertible), "Nam" (INTEGER), "Thoi gian" (TEXT, e.g., 'Tháng 01').
    -- 3. "Doanh_thu" table has "Tên đơn vị" (TEXT), "Kỳ báo cáo" (numeric convertible), "Năm" (INTEGER), "Tháng" (TEXT, e.g., 'Tháng 01').
    -- 4. Location names in "Fulltime"."dia_diem", "Parttime"."Don vi", and "Doanh_thu"."Tên đơn vị" are comparable for grouping.
    -- 5. The exclusion list for "Doanh_thu"."Tên đơn vị" is intentional.
    ...
    SELECT
        ...
        CASE
            WHEN COALESCE(rev.total_revenue, 0) = 0 THEN 0.0 -- Return 0 for component if no revenue or revenue is zero
